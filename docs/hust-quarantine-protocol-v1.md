...
目前只完成到第 1 道门，并生成了等待确认的候选清单。没有人工批准时，代码会拒绝将它标记为可转换。
...
被 Git 忽略的 `data/HUST/` 目录已有 77 个解出的 pickle。它们还没有逐文件证明与 ZIP 成员哈希一致，也不符合“原始 pickle 位于工作区外”的目标边界。

在用户明确同意之前，不移动、不删除这些文件；主应用、Docker 构建和 A100 上传必须继续排除该目录。

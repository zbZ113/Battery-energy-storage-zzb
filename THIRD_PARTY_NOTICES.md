...
MATR、Naumann 及其他外部数据只在对应数据卡记录的授权范围内使用。每个快照必须登记正式名称、发布方、原始链接、引用、条款版本、获取日期、是否允许再分发及文件哈希。仓库不直接包含这些原始数据。
...
- BatteryML（Microsoft，MIT License）：MATR 原始字段语义和电芯—周期分层结构的概念参考。目标仓库的数据适配、校验、划分和安全持久化均为独立实现，不复制其 pickle 持久化、Severson 特征或划分逻辑。许可证原文位于本地审计工程 `BatteryML-main/BatteryML-main/LICENSE`，正式发布时随 SBOM 固化准确版本与版权声明。

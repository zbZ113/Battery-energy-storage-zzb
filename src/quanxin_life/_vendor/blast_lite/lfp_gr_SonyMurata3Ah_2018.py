...
class Lfp_Gr_SonyMurata3Ah_Battery(BatteryDegradationModel):
    ...
    def update_rates(self, stressors: dict):
        ...
        * np.exp(p['q1_b2']*(1/TdegK)*(Ua**0.5))
        ...
        q1 = np.trapezoid(q1, x=t_secs) / delta_t_secs
        q3 = np.trapezoid(q3, x=t_secs) / delta_t_secs
        # q5 = np.trapezoid(q5, x=t_secs) / delta_t_secs # no time varying inputs
        q7 = np.trapezoid(q7, x=t_secs) / delta_t_secs
        k_temp_r_cal = np.trapezoid(k_temp_r_cal, x=t_secs) / delta_t_secs
        k_soc_r_cal = np.trapezoid(k_soc_r_cal, x=t_secs) / delta_t_secs
        # k_Crate_r_cyc = np.trapezoid(k_Crate_r_cyc, x=t_secs) / delta_t_secs # no time varying inputs
        # k_dod_r_cyc = np.trapezoid(k_dod_r_cyc, x=t_secs) / delta_t_secs # no time varying inputs
    ...
    def update_states(self, stressors: dict):
        ...
        # Resistance
        dr_LLI_t = self._degradation_scalar * r['r_kcal'] * delta_t_days * 86400
        dr_LLI_EFC = self._degradation_scalar * r['r_kcyc'] * delta_efc / 100

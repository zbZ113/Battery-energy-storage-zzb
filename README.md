...
- FastAPI项目、数据集、实验登记、安全模型制品登记、Agent运行、知识库与飞书事件接口；审核知识材料支持幂等向量索引、pgvector/BM25/reranker混合检索和整库显式降级。
...
- A100结果安全导入已具备完整矩阵、来源提交、传输哈希和逐文件SHA-256门禁；已导入套件可幂等登记到PostgreSQL，并按项目、数据集、模型、截断点和种子统一查询。已验证JSON/UBJ模型可由管理员幂等登记到项目制品目录；A100任务制品到该安全来源的自动映射及safetensors服务加载仍待正式结果回传后贯通。

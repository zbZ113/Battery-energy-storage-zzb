"""Build and immediately verify the three-batch MATR A100 ZIP64 package."""
...
def main() -> int:
    ...
    if verified != manifest:
        raise ValueError("new three-batch package failed immediate internal verification")
    ...
    print(
        json.dumps(
            {
                "status": "MATR_THREE_BATCH_A100_PACKAGE_READY",
                "archive": str(output),
...
def _require_clean_git(root: Path) -> None:
    if _git_output(root, ["git", "status", "--porcelain"]):

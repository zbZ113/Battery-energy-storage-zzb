"""Verify the external and internal three-batch A100 package before extraction."""
...
def main() -> int:
    ...
    manifest = verify_matr_three_batch_a100_archive(args.archive)
    ...
    print(
        json.dumps(
            {
                "status": "MATR_THREE_BATCH_A100_PACKAGE_VERIFIED",
                "archive_sha256": index.archive_sha256,
                "package_sha256": manifest.package_sha256,
                "source_commit": manifest.source_commit,
                "file_count": len(manifest.files),
                "raw_batch_count": len(manifest.raw_matr_sha256),
            },
            ensure_ascii=False,
        )
    )
